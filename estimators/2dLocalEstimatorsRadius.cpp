                         ...
                         double noiseLevel = 0.0 )
{
  ...
  try {
    ...
    if (gridcurve.isClosed())
    {
      ...
      double timeSegmentGlobal = c.stopClock();
      ...
      double timeSegmentLocal = c.stopClock();
      ...
      if (options.at(4) != '0')
      {
        if( curvature )
        {
          ...
          {
            char full_filename[360];
            sprintf( full_filename, "%s%s", filename.c_str(), "_global_timings.dat" );
...
//            estimator.eval( contour.begin(), contour.end(), out_it );
          ...
          {
            char full_filename[360];
            sprintf( full_filename, "%s%s%u%s", filename.c_str(), "_local_timings_with_", optionsII.nbKernels , "kernels.dat" );
            std::ofstream file( full_filename, std::ofstream::app );
...
//            trace.endBlock();
            ...
            double time = c.stopClock() + timeSegmentLocal;
            file << h << " " << time / 1000.0 <<  std::endl;
